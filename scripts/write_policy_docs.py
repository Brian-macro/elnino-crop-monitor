"""Generate the human-readable stitching plan from the same executable policy."""

from pathlib import Path
from policy import POLICY, policy_bundle
...
def main():
    ...
    lines = [
        "# 数据来源与历史/预测拼接方案",
        "",
        f"政策版本：`{POLICY['version']}`。唯一配置：`config/research_policy.json`。",
        "",
        "## 研究单元与主来源",
        "",
    ...
    for crop, members in POLICY["regions"]["Southeast Asia"]["members_by_crop"].items():
        lines.append(
            f"| {crop} | {', '.join(members) or '不设汇总项：入选生产规模不足'} | {'PSD MY；精米' if crop=='rice' else 'PSD MY；产量求和'} |"
        )
    lines += [
        "",
        "## 小产量剔除规则",
    ...
    for crop, countries in POLICY["active_countries"].items():
        lines.append(f"| {crop} | {', '.join(countries)} |")
    lines += [
        "",
        "## 代码与自动更新",
        "",
    ...
    path = Path(__file__).resolve().parents[1] / "docs" / "DATA_STITCHING.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(path)
