...
def main():
    ...
    lines = ["# 来源配置矩阵", "", f"政策版本：{bundle['version']}", "",
             "本表为配置，不代表对应年度已采用；实际成对选择、证据与PSD回退见dashboard JSON。方法见DATA_STITCHING.md。", "",
             "| 作物 | 国家/地区 | 历史基础 | 预测优先 | 注册本土来源 | 回退原因 |",
             "|---|---|---|---|---|---|"]
    for row in bundle['source_matrix']:
        lines.append(f"| {row['crop']} | {row['unit']} | {row['history']} | {row['forecast']} | {row['configured_local_source'] or '—'} | {row['fallback_reason'] or '缺少可比两年值时明确回退PSD'} |")
